...
DIASTEMA_PORT = int(os.getenv("DIASTEMA_PORT", 5001))

# Diastema Analytcs API Host and Port
DIASTEMA_DATA_LOADING_HOST = os.getenv("DIASTEMA_DATA_LOADING_HOST", "localhost")
DIASTEMA_DATA_LOADING_PORT = int(os.getenv("DIASTEMA_DATA_LOADING_PORT", 5001))
DIASTEMA_DATA_CLEANING_HOST = os.getenv("DIASTEMA_DATA_CLEANING_HOST", "localhost")
DIASTEMA_DATA_CLEANING_PORT = int(os.getenv("DIASTEMA_DATA_CLEANING_PORT", 5001))
...
diastema_front_end_host = "http://"+DIASTEMA_HOST+":"+str(DIASTEMA_PORT)+"/messages"

# Variable to see jobs not ready yet
running_jobs = {}
...
# A function to wait for a job to finish before moving on the next deaper job in the tree graph
def waitForJob(job_id):
    while(True):
...
def data_load(playbook, job, data_set_files):
    ...
    # hold job on running
    running_jobs[minioString(job["id"])] = True
